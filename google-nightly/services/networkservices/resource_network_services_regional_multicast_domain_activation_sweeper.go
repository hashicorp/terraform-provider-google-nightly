...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesRegionalMulticastDomainActivation,
		DeleteFunction: testSweepNetworkServicesRegionalMulticastDomainActivation,
	}
...
func testSweepNetworkServicesRegionalMulticastDomainActivation(_ string) error {
	return listAndActionNetworkServicesRegionalMulticastDomainActivation(deleteResourceNetworkServicesRegionalMulticastDomainActivation)
}

func listAndActionNetworkServicesRegionalMulticastDomainActivation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesRegionalMulticastDomainActivation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1alpha1/projects/{{project}}/locations/{{location}}/regionalMulticastDomainActivations", "?")[0]
		...
		resourceList, ok := res["regionalMulticastDomainActivations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'regionalMulticastDomainActivations'")
		} else {
...
func deleteResourceNetworkServicesRegionalMulticastDomainActivation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1alpha1/projects/{{project}}/locations/{{location}}/regionalMulticastDomainActivations/{{regional_multicast_domain_activation_id}}"
