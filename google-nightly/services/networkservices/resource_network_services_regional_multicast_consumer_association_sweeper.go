...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesRegionalMulticastConsumerAssociation,
		DeleteFunction: testSweepNetworkServicesRegionalMulticastConsumerAssociation,
	}
...
func testSweepNetworkServicesRegionalMulticastConsumerAssociation(_ string) error {
	return listAndActionNetworkServicesRegionalMulticastConsumerAssociation(deleteResourceNetworkServicesRegionalMulticastConsumerAssociation)
}

func listAndActionNetworkServicesRegionalMulticastConsumerAssociation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesRegionalMulticastConsumerAssociation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1alpha1/projects/{{project}}/locations/{{location}}/regionalMulticastConsumerAssociations", "?")[0]
		...
		resourceList, ok := res["regionalMulticastConsumerAssociations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'regionalMulticastConsumerAssociations'")
		} else {
...
func deleteResourceNetworkServicesRegionalMulticastConsumerAssociation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1alpha1/projects/{{project}}/locations/{{location}}/regionalMulticastConsumerAssociations/{{regional_multicast_consumer_association_id}}"
