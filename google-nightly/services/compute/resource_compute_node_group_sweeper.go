...
func listAndActionComputeNodeGroup(action sweeper.ResourceAction) error {
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/alpha/projects/{{project}}/aggregated/nodeGroups", "?")[0]
...
func deleteResourceComputeNodeGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/alpha/projects/{{project}}/zones/{{zone}}/nodeGroups/{{name}}"
	if obj["zone"] == nil {
