...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeHaController,
		DeleteFunction: testSweepComputeHaController,
	}
...
func testSweepComputeHaController(_ string) error {
	return listAndActionComputeHaController(deleteResourceComputeHaController)
}

func listAndActionComputeHaController(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeHaController"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/alpha/projects/{{project}}/regions/{{region}}/haControllers", "?")[0]
		...
		resourceList, ok := res["haControllers"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'haControllers'")
		} else {
...
func deleteResourceComputeHaController(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/alpha/projects/{{project}}/regions/{{region}}/haControllers/{{name}}"
