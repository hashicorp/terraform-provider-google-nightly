...
func listAndActionComputeRoute(action sweeper.ResourceAction) error {
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/alpha/projects/{{project}}/global/routes", "?")[0]
...
func deleteResourceComputeRoute(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/alpha/projects/{{project}}/global/routes/{{name}}"
