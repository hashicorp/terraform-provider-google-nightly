...
func listAndActionComputeAutoscaler(action sweeper.ResourceAction) error {
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/alpha/projects/{{project}}/aggregated/autoscalers", "?")[0]
...
func deleteResourceComputeAutoscaler(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/alpha/projects/{{project}}/zones/{{zone}}/autoscalers/{{name}}"
	if obj["zone"] == nil {
